...
Figure~\ref{fig:cvrcPODEnergy} displays the POD residual energy for the model rocket combustor data. Achieving 1\%, 0.1\%, and 0.01\% residual energy for the conservative dataset requires 93, 227, and 473 basis modes, respectively. For the primitive dataset, these levels require 81, 239, and 517 modes respectively. This decay is remarkably slower than that observed for 2D open cavity flow (Fig.~\ref{fig:cavityPODEnergy}), and is another of the difficult with which linear trial spaces capture the highly non-linear flow physics which characterize rocket combustion. This is further emphasized in the projection error plots shown in Figs.~\ref{fig:cvrcProjErrPrim} and~\ref{fig:cvrcProjErrCons}, where we see that
