...
% Moving forward, for the sake of clarity to the broader ROM community (which, by and large, does not use non-linear trial manifolds), we will retain the linear representation of the approximate state given by Eq.~\ref{eq:consSolApprox} in deriving ROMs in Sections~\ref{sec:classicROMs}--\ref{sec:adaptation}. We will provide derivations using general trial manifold representations in Section~\ref{sec:nonlinRoms}.
