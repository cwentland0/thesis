...
\begin{frame}{Least-squares Petrov--Galerkin (LSPG)\footnotemark[1]}
    \begin{itemize}
		...
		\end{equation*}
	\end{itemize}
	\footnotetext[1]{\tiny{Carlberg el al., \textit{Int. J. Numer. Meth. Engng}, Vol. 86, 2010, pp. 155--181.}}
\end{frame}

\begin{frame}{State of the art in 2017}
	\begin{itemize}
		\item PROMs for parametrized elliptic problems are extremely robust\footnotemark[2]
		\item Galerkin ROMs are generally unstable for unsteady problems
		\item LSPG is leap forward, biggest case in 3D Ahmed body\footnotemark[3]
		\item First studies of PROMs for reacting flows suffer from instabilities\footnotemark[4]
	\end{itemize}
	...
	\footnotetext[2]{\tiny{Hesthaven et al., \textit{Certified Reduced Basis Methods for Parametrized Partial Differential Equations}, Springer, 2015.}}
	\footnotetext[3]{\tiny{Carlberg et al., \textit{J Comp Phys}, Vol. 242, 2013, pp. 623--647.}}
	\footnotetext[4]{\tiny{Huang et al., \textit{AIAA Propulsion and Energy}, 2018.}}
\end{frame}

\begin{frame}{Practical limitations of Galerkin and LSPG}
    \begin{itemize}
		\item Conservative variables are cumbersome with more complex gas models
		\begin{equation*}
			p = \rho RT
		\end{equation*}
		\begin{equation*}
			E = \frac{1}{2} \mathbf{u} \cdot \mathbf{u} + h - \frac{p}{\rho}
		\end{equation*}
		\begin{equation*}
			\frac{h(T)}{RT} = a_1 + \frac{a_2}{2}T + \frac{a_3}{3}T^2 + \frac{a_4}{4}T^3 + \frac{a_5}{5}T^4 + \frac{a_6}{T}
		\end{equation*}
		\item Enforcing global limiters
		\begin{equation*}
			\consVecRom \rightarrow \mathbf{T} \rightarrow max(\mathbf{T}, T_{max}) \rightarrow \consVecRom 
		\end{equation*}
...
\begin{frame}{\small{Model-preserving least-squares with variable transformation (MP-LSVT)\footnotemark[2]}}
	\begin{itemize}
	...
	\begin{minipage}{0.49\linewidth}
		...
		\end{figure}
	\end{minipage}
	\footnotetext[2]{\tiny{Huang et al., \textit{J Comp Phys.}, Vol. 448, 2022, pp. 110742.}}
\end{frame}
...
\begin{frame}{Least-squares regression \& interpolation}
    \begin{itemize}
		...
		\end{align*}
		\item Least-squares regression, gappy POD\footnotemark[3] ($\numSamps > \numResModes$)
		\begin{equation*}
		...
		\end{equation*}
		\item Discrete empirical interpolation method (DEIM)\footnotemark[4] ($\numSamps = \numResModes$)
		\begin{equation*}
		...
		\end{equation*}
	\end{itemize}
	\footnotetext[3]{\tiny{Everson and Sirovich, \textit{J Opt Soc Am A}, Vol. 12 (8), 1995, pp 1657--1664.}}
	\footnotetext[4]{\tiny{Chaturantabut and Sorensen, \textit{SIAM J Sci Comput}, Vol. 32 (5), 2010, pp. 2737--2764.}}
\end{frame}

\begin{frame}{Sampling algorithms}
    \begin{itemize}
		\item Computing optimal sampling $\sampMat$ is NP-hard
		\item Usually resort to greedy (locally-optimal) algorithms
		\begin{itemize}
			\item DEIM\footnotemark[4], GNAT\footnotemark[5]
			\item Accelerated MPE\footnotemark[6], GPOD+E\footnotemark[7]
			\item Determinant-based\footnotemark[8]
		\end{itemize}
	\end{itemize}
	\begin{minipage}{0.13\linewidth}
		...
		\vspace{4em}
	\end{minipage}\hfill
	\begin{minipage}{0.84\linewidth}%{0.84\linewidth}
		\includegraphics[width=0.99\linewidth]{theory/iBlanks.jpg}
	\end{minipage}
	\vspace{-2em}
	\footnotetext[5]{\miniscule{Carlberg et al., \textit{J Comp Phys}, Vol. 242, 2013, pp. 623--647.} \scriptsize{\textsuperscript{6}}\miniscule{Zimmermann and Willcox, \textit{SIAM J Sci Comp}, Vol. 38, 2016, pp. 2827--2850.}}
	\footnotetext[7]{\miniscule{Peherstorfer et al., \textit{SIAM J Comp Sci}, Vol. 42, 2020, pp. 2837--2864.} \scriptsize{\textsuperscript{8} \miniscule{Saito et al., \textit{IEEE Access}, Vol. 9, pp. 68535--68551}}}
\end{frame}
...
\begin{frame}{Numerical Experiments}
    \begin{itemize}
		\item The General Equations and Mesh Solver (GEMS)\footnotemark[9]
		\begin{itemize}
	...
	\end{equation*}
	\normalsize
	\footnotetext[9]{\tiny{Li et al., \textit{3rd Int Conf CFD}, 2006, pp. 619--624.}}
\end{frame}
...
\begin{frame}{2D transonic flow over an open cavity}
	...
	\begin{minipage}{0.59\linewidth}
		\begin{figure}
			\animategraphics[autoplay,loop,trim=1em 1em 1em 1em,width=0.99\linewidth]{10}{experiments/2d_cavity/fom_animation/fig_}{000001}{000052}
		\end{figure}
	\end{minipage}
\end{frame}
...
\begin{frame}{3D truncated CVRC\footnotemark[10]}
    \begin{itemize}
		...
		\item No acoustic forcing (for now)
	\end{itemize}
	\vspace{0.5em}
	\begin{minipage}{0.47\linewidth}
		\begin{figure}
			\animategraphics[autoplay,loop,trim=1em 1em 1em 1em,width=0.99\linewidth]{10}{experiments/cvrc/temp_animation/fig_}{000001}{000050}
		\end{figure}
	\end{minipage}
	\begin{minipage}{0.51\linewidth}
		\begin{figure}
			\includegraphics[width=0.99\linewidth]{experiments/cvrc/point_8_Static_Pressure_FOM.png}
		\end{figure}
	\end{minipage}
	\footnotetext[10]{\tiny{Yu et al., \textit{J Propul Power}, Vol. 28 (5), 2012, pp. 876--887}}
\end{frame}

\begin{frame}{3D truncated CVRC}
	\begin{itemize}
		\item Stable and (mostly) accurate even with $\dt = 10\dt_{FOM}$ 
	\end{itemize}
	\vspace{1em}
    \begin{minipage}{0.44\linewidth}
		\begin{figure}
			\includegraphics[width=0.99\linewidth]{experiments/cvrc/point_8_Static_Pressure_k100.png}
		\end{figure}
	\end{minipage}
	\begin{minipage}{0.55\linewidth}
		\begin{figure}
			\animategraphics[autoplay,loop,trim=1em 1em 1em 1em,width=0.99\linewidth]{10}{experiments/cvrc/temp_slices_rom/fig_merged_}{1}{50}
		\end{figure}
	\end{minipage}
\end{frame}
...
\begin{frame}{3D truncated CVRC}
    \begin{figure}
		\animategraphics[controls={step},buttonsize=1.0em,width=0.6\linewidth]{10}{experiments/cvrc/pareto_PLACEHOLDER/samplingTypeStudy_Average_errorRaw_time_calcAndMPI_pareto_}{1}{3}
	\end{figure}
	\begin{tcolorbox}[colframe=blue!50!white,halign=center]
...
\begin{frame}{3D nine-element combustor\footnotemark[11]}
	...
	\begin{minipage}{0.45\linewidth}
		\begin{figure}
			\animategraphics[final,autoplay,loop,trim=1em 1em 1em 1em,width=0.99\linewidth]{10}{experiments/9elem/temp_slices/fig_}{000001}{000040}
		\end{figure}
	\end{minipage}
	\begin{minipage}{0.53\linewidth}
		...
		\vspace{-3em}
	\end{minipage}
	\footnotetext[11]{\tiny{Harvazinski et al., \textit{AIAA SciTech}, 2019}}
\end{frame}
...
\begin{frame}{Misc: The Adjoint Petrov--Galerkin method\footnotemark[12]}
	\begin{itemize}
	...
	\begin{minipage}{0.49\linewidth}
		...
		\end{figure}
	\end{minipage}
	\footnotetext[12]{\tiny{Parish et al., \textit{Comput Method Appl M}, Vol. 365, 2020, pp. 112991.}}
\end{frame}

\begin{frame}{\footnotesize{Misc: Prototyping Environment for Reacting Flow Order Reduction Methods (PERFORM)\footnotemark[13]}}
	\begin{itemize}
		\small
	...
	\footnotetext[13]{\tiny{Wentland, \textit{J Open Source Soft.}, Under review.}}
\end{frame}

\begin{frame}{Misc: Non-linear manifold ROMs\footnotemark[14]}
	\begin{itemize}
	...
	\begin{minipage}{0.49\linewidth}
		...
		\end{figure}
	\end{minipage}
	\footnotetext[14]{\tiny{Wentland et al., \textit{74th APS DFD}, Vol. 66 (17), 2021.}}
\end{frame}
...
\begin{frame}{Misc: HPROM toolchain (contrib. to PDP)}
	...
	\begin{itemize}
		\item Parallel Data Processor\footnotemark[15] developed by Nicholas Arnold-Medabalimi
		...
		\end{itemize}
	\end{itemize}
	\footnotetext[15]{\tiny{Arnold-Medabalimi, \textit{SoftwareX}, In preparation.}}
\end{frame}
